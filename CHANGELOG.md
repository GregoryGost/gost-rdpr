...
- SQLAlchemy async ORM for SQLite DB. Insertion, updating, and deletion are now performed in a single queue, since the
 database is a single file and locks must be avoided
...
- Add `/config` method for Home section. Prints the current configuration. Static and dynamic settings
...
- Domain length validation has been removed. Instead, shorter domains are now rejected before being saved to the
 database to avoid acceptance errors
- Domains black list
...
- Change ips lists load commands. Now this is a single command for loading all lists, both domains and IP addresses.
 New `/commands/lists/load`. Old `/commands/domains/lists/load` and `/commands/ips/lists/load`
...
- Removed `librouteros`. Reason: Why maintain an additional module when you can use the existing `httpx` module to work
 with the ROS REST API (available since version 7.9). You need to enable the web service and grant rights to the user.
